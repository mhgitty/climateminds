...
import { getChildPageBySlug } from '@/lib/sanity'
...
interface Props { params: Promise<{ parent: string; slug: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { parent, slug } = await params
  const page = await getChildPageBySlug(parent, slug).catch(() => null)
  if (!page) return {}
  const title = page.metaTitle || page.title
  const description = page.metaDescription || page.intro || ''
  const canonical = `${BASE}/${parent}/${slug}/`
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: page.parentTitle || parent, item: parentUrl },
